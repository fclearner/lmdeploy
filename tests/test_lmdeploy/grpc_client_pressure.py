...
def record_response(counters: Counters, item: WorkItem, body: dict[str, Any], latency_ms: float) -> None:
    ...
    if server_total_ms is not None:
        counters.client_minus_server_ms.append(max(0.0, latency_ms - server_total_ms))
...
async def run_phase(name: str, stub: Any, args: argparse.Namespace, items: list[WorkItem]) -> Counters:
    ...
    print_summary(name, counters, elapsed)
    return counters
...
def print_summary(name: str, counters: Counters, elapsed_sec: float) -> None:
    print(
    ...
    print(
        ...
        f"max={fmt(max(counters.latencies_ms) if counters.latencies_ms else None)}"
    )
    print(
        "  server_ms  "
